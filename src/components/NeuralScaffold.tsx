...
interface NeuralScaffoldProps {
  ...
  viewport: Viewport;
  patternType?: 'dots' | 'grid' | 'none'; // New prop for patterns
}
...
export default function NeuralScaffold({ 
  ...
  connections = [],
  viewport,
  patternType = 'none' // Default to no pattern
}: NeuralScaffoldProps) {
  ...
  const render = useCallback(() => {
    ...
    if (patternType && patternType !== 'none') {
      drawPattern(ctx, patternType, isDarkMode);
    }
  ...
  }, [width, height, isDarkMode, patternType]);

  // Helper function to draw patterns
  const drawPattern = useCallback((ctx: CanvasRenderingContext2D, type: 'dots' | 'grid', isDarkMode: boolean) => {
    const patternColor = isDarkMode ? 'rgba(255, 255, 255, 0.05)' : 'rgba(0, 0, 0, 0.05)';
    const spacing = 50;
