...
export default function SettingsPage({ isDarkMode, onNavigate }: SettingsPageProps) {
  ...
  // State to manage the scroll position, if needed for complex scenarios
  ...
  const handleSaveKeys = async () => {
    ...
    try {
      ...
      console.log('Saving API keys securely:', keysToSave.map(k => ({ provider: k.provider, keyLength: k.key.length })));
  ...
  return (
    <div className={`flex flex-col h-screen ${isDarkMode ? 'bg-black' : 'bg-stone-50'}`}>
      {/* Header */}
      <div className={`
